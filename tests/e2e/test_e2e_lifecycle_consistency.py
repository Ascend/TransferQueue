...
BACKEND_CONFIGS = {
    ...
    # use_gdr=True requires protocol="rdma" so that Mooncake can perform true
    # GPUDirect RDMA transfers directly from the registered CUDA staging buffer.
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required for GDR test")
def test_gdr_cpu_tensor_roundtrip(e2e_client, backend_name):
    """GDR path: put CPU tensors, get back and verify data, clear and verify gone.

    Only meaningful when backend_name == "MooncakeStore_GDR"; skipped otherwise.
    Tests the H2D staging-buffer path: H2D pack → RDMA PUT → RDMA GET → D2D unpack.
    Verifies that CPU tensors are transparently routed through GDR when use_gdr=True.
    """
    if backend_name != "MooncakeStore_GDR":
        pytest.skip("GDR CPU roundtrip only runs with TQ_TEST_BACKEND=MooncakeStore_GDR")

    client = e2e_client
    partition_id = "test_gdr_cpu_tensor_roundtrip"
    task_name = "gdr_cpu_roundtrip_task"
    batch_size = 8

    # Build a TensorDict with CPU tensors — these should be H2D-copied into the
    # staging buffer and transferred via GDR, not the CPU RDMA path.
    indices = list(range(batch_size))
    cpu_data = TensorDict(
        {
            "cpu_f32": torch.stack([torch.arange(i, i + 64, dtype=torch.float32) for i in indices]),
            "cpu_bf16": torch.stack([torch.full((32,), float(i), dtype=torch.bfloat16) for i in indices]),
            "cpu_i64": torch.stack([torch.arange(i * 10, i * 10 + 8, dtype=torch.int64) for i in indices]),
        },
        batch_size=batch_size,
    )

    meta = client.put(data=cpu_data, partition_id=partition_id)
    assert meta.size == batch_size

    try:
        retrieved_meta = poll_for_meta(client, partition_id, list(cpu_data.keys()), batch_size, task_name, mode="fetch")
        assert retrieved_meta is not None and retrieved_meta.size == batch_size, "Failed to retrieve GDR CPU metadata"
        retrieved = client.get_data(retrieved_meta)

        for i in range(batch_size):
            assert torch.equal(retrieved["cpu_f32"][i].cpu(), cpu_data["cpu_f32"][i]), f"cpu_f32 mismatch at index {i}"
            assert torch.equal(retrieved["cpu_bf16"][i].cpu(), cpu_data["cpu_bf16"][i]), (
                f"cpu_bf16 mismatch at index {i}"
            )
            assert torch.equal(retrieved["cpu_i64"][i].cpu(), cpu_data["cpu_i64"][i]), f"cpu_i64 mismatch at index {i}"
