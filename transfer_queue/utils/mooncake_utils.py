...
class GdrStaging:
    ...
    Design note: the single-buffer-per-process design is intentional for RL workloads,
    where put/get calls are bursty but not highly concurrent (training and inference
    alternate in phases). The single buffer avoids repeated cudaMalloc/register overhead
    and keeps HBM usage bounded. For high-concurrency use cases, a buffer pool would be
    needed to avoid serialization through the lock.
    """
    ...
    def lazy_init(self, store) -> None:
        """Import cuda-python, cudaMalloc, register_buffer; idempotent and thread-safe."""
        if self._initialized:
            return
        with self._lock:
            if self._initialized:  # re-check under lock to guard concurrent first calls
                return
            try:
                from cuda.bindings import driver as cuda_driver
                from cuda.bindings import runtime as cudart
            except ImportError:
                from cuda import cuda as cuda_driver  # type: ignore[no-redef]
            ...
            err, supported = cuda_driver.cuDeviceGetAttribute(
                cuda_driver.CUdevice_attribute.CU_DEVICE_ATTRIBUTE_GPU_DIRECT_RDMA_SUPPORTED,
                device_ordinal,
            )
            if err != cuda_driver.CUresult.CUDA_SUCCESS:
                raise RuntimeError(f"cuDeviceGetAttribute() failed: {err.name}")
            if not supported:
                raise RuntimeError(
    ...
    def wait_for_current_stream(self) -> None:
        """Make the staging stream wait for pending work on the caller's current PyTorch stream."""
        assert self._stream is not None
