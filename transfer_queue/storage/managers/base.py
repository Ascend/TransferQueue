...
class TransferQueueStorageManager(ABC):
    ...
    async def notify_data_update(
        ...
        custom_meta: dict[int, dict[str, Any]],
    ) -> None:
        """
...
class KVStorageManager(TransferQueueStorageManager):
    ...
    def _get_shape_type_custom_meta_list(metadata: BatchMeta):
        ...
        custom_metas = metadata._get_all_custom_meta()

        if custom_metas:
            custom_metas = [
                custom_metas.get(index, {}).get(field_name, None)
                for field_name, index in itertools.product(
                    sorted(metadata.field_names), range(len(metadata))
                )
            ]

        return shapes, dtypes, custom_metas
            ...
            for global_idx in metadata.global_indexes:
                ...
                for field_name in metadata.field_names:
                    per_field_custom_meta[global_idx][field_name] = (
                        custom_meta.get(self._generate_key(field_name, global_idx), None)
                    )
            metadata._update_custom_meta(per_field_custom_meta)
